...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if pad_size > 0:
            new_sent = sentence + ( [zero_vector] * pad_size)
            new_label = labels + ( [zero_label] * pad_size)
            mask =  [True] * len(labels)
            mask.extend([False]*pad_size)


            ret.append((new_sent,new_label,mask))
        else :

            new_sent = sentence[:max_length]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        print("************State Shape :: {} X {}".format(x.shape,Config.hidden_size))
        # Outputs - ************State Shape :: (?, 52, 300) X 300
